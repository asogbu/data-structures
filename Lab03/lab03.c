...
int main(const int argc, const char* argv[])
{
   ...
   if (argc != 5) {
	  ...
	  fprintf(stdout, "This program expects four arguments: row, column, fill character, and name of file\n");
      exit (-1);
   }
...
void Flood (char box[MAX_ROWS][MAX_COLS], int row, int col, char fill)
{
   ...
   */
	if (row > 0) Flood(box, row - 1, col, fill);
	if (row < MAX_COLS) Flood(box, row + 1, col, fill);
	if (col > 0) Flood(box, row, col - 1, fill);
	if (col < MAX_COLS) Flood(box, row, col + 1, fill);
}
