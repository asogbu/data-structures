...
void find( VECTOR< VECTOR<STRING> >& solutions, VECTOR<STRING>& current_partitions, const STRING& the_string ){
	SIZE_T the_size = the_string.size();
	...
	for (SIZE_T iter = 1; iter <= the_size; ++iter) {
		STRING the_substring = the_string.substr(0, iter);
		if (is_palindrome(the_substring)) {
			current_partitions.push_back(the_substring);
