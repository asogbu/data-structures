...
struct Graph{
	...
	public:
		...
		void BFS( unsigned int origin, unsigned int destin ){
			 ...
			 *****************************************************/
			 if ((this->vertices.size() == 0) || (origin >= this->vertices.size()) || (destin >= this->vertices.size())) {
			 	COUT << "Invalid BFS inputs" << ENDL;
				return;
			 }
			...
			unsigned parents[this->vertices.size()];
			parents[origin] = -1;
			...
			while (!found && theQueue.size() > 0) {
				Vertex<unsigned int> vertex(theQueue.front());
				theQueue.pop();
				visited[vertex.get_vertex_value()] = true;

				for (unsigned int iter = 0; iter < this->vertices[vertex.get_vertex_value()].num_edges(); ++iter) {
					unsigned int edgeDestin = this->vertices[vertex.get_vertex_value()].get_edge(iter).destin;

					if (edgeDestin == destin) {
						found = true;
						parents[destin] = vertex.get_vertex_value();
						break;
					}

					if (!visited[edgeDestin]) {
						theQueue.push(edgeDestin);
						parents[edgeDestin] = vertex.get_vertex_value();
						visited[edgeDestin] = true;
					}
				}
			} 
